...
import akka.NotUsed
import akka.actor.{Actor, ActorLogging, ActorPath, ActorRef, ActorSelection, Props, RootActorPath}
...
import io.flashbook.flashbot.core.{DataPath, DeltaFmt, DeltaFmtJson, MarketData}
import io.flashbook.flashbot.core.DeltaFmt._
...
class DataServer(marketDataPath: File,
                 ...
                 ingestMatchers: Set[DataPath]) extends Actor with ActorLogging {
  import DataServer._
  ...
  val localDataSourceActors = configs.map {
    case (key, config) =>
      val ref = context.system.actorOf(Props(
        new DataSourceActor(marketDataPath, key, config)), key)
      // If there are any matchers defined, start the ingest process immediately.
      if (ingestMatchers.nonEmpty) {
        ref ! Ingest(ingestMatchers)
      }
      key -> ref
  }
